...
tmux new-session -d -s telescope -n main "bash -c 'uvicorn main:app --host 0.0.0.0 --port 8000; exec bash'"
tmux split-window -v "bash -c 'cloudflared tunnel run telescope-api; exec bash'"
